class MessageHandler {
  ...
  startNewTurn(room, drawerId, wordOptions) {
    ...
    if (!drawer || !wordOptions?.length) return;
    ...
    room._wordPickTimeout = setTimeout(() => {
      if (room.game && room.game.phase === 'word_selection') {
        ...
        this.handleWordChosen({ id: drawerId }, { word: autoWord }, room);
      }
    }, 15000);
  }
  ...
  handleRoundEnd(room) {
    ...
    if (!wordOptions) {
      room.game = null;
      room.broadcast('game_cancelled', {
        message: 'Game cancelled because there are not enough players.',
        players: room.getPlayersList()
      });
      return;
    }
  ...
  handleGetPublicRooms(socket) {
    ...
    this.rooms.forEach((room) => {
      ...
      if (!room.settings.isPrivate && !room.isFull() && !roomData.inProgress) {
